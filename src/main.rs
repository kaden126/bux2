    ...
    match cli.command {
        Commands::Build { profile } => {
            process::build(&profile)
                .with_context(|| "Failed to run build process.")?;
        },
        Commands::Generate => {
            bux2::generate_build_files()
                .with_context(|| "Failed to generate build files.")?;
        },
        ...
        Commands::DeinitProfile { profile } => {
            process::deinit(&profile)
                .with_context(|| "Failed deinitialize profile.")?;
        },
        Commands::Fetch { profile } => {
            process::fetch(&profile)
        ...
        Commands::New { name } => {
            process::new(&name)
                .with_context(|| "Failed to generate new project.")?;
        }
    }
