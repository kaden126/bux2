...
pub fn write_manifest(config: &Config) -> anyhow::Result<()> {
    ...
    writeln!(
        &mut file,
        ": 1\nname: {}\nversion: {}\nlicense: {}\nsummary: {}\n",
        config.package.name, config.package.version, config.package.license, config.package.summary,
    )
    ...
    if let Some(keywords) = &config.package.keywords {
        ...
        writeln!(&mut file, "").with_context(|| format!("Failed to write {}.", path.display()))?;
    }
